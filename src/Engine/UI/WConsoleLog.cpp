		...
		if(m_Lines[lineNo].size() > 0) {
			...
			if(	(iLeftTop < m_maxX && iLeftTop >= m_minX) || ( iRightTop < m_maxX && iRightTop >= m_minX )) {
				if(c > ' ') {
					...
					//Rect charRect(xX, yY, CHAR_WIDTH, WWidgetManager::CHARACTER_HEIGHT);
					//renderer->fillRect(0.0f, 255.0f, 255.0f, 255.0f, &charRect);
					...
					//renderer->drawHorizontalLine(0.0f, 0.0f, 0.0f, 255.0f, xX, yY + WWidgetManager::CHARACTER_HEIGHT - 2, CHAR_WIDTH, 1);
					//renderer->drawHorizontalLine(0.0f, 0.0f, 0.0f, 255.0f, xX, yY + (WWidgetManager::CHARACTER_HEIGHT >> 1), CHAR_WIDTH, 1);
...
void WConsoleLog::onRender() {
	...
	renderer->fillRect(255.0f, 255.0f, 255.0f, 255.0f, &thisWndRect);
	renderer->drawRect(0.0f, 0.0f, 0.0f, 255.0f, &thisWndRect);
	...
	{	
			...
			if(mState == READONLY) {
				...
				renderer->fillRect(128.0, 128.0f, 128.0f, 255.0f, &readOnlyRect);
			}
	}
	resetClip();
	////////////////////////////////////////////////////////
...
void WConsoleLog::onKeyBDown(unsigned int iVirtualKeycode, unsigned short ch) {
...
void WConsoleLog::onKeyBUp(unsigned int iVirtualKeycode, unsigned short ch) {

}

void WConsoleLog::onMessage(UINT msg, WPARAM wParam, LPARAM lParam) {
	switch(msg) {
		case MOUSE_DOWN:
		{
			int buttonID = wParam;
