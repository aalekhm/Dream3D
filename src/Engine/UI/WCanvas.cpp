...
void WCanvas::onCreateEx(LPVOID lpVoid) {
	...
	if(m_pCameraNode == NULL) {
		Camera* pCanvasCamera = Camera::createPerspective(getLeft(), getTop(), getWidth(), getHeight(), 45.0f, 0.01f, 10.0f);
		m_pCameraNode = Node::create("YAGUI_2DCamera");
		m_pCameraNode->setCamera(pCanvasCamera);
	}
}
...
void WCanvas::setVScrollbarLength() {
...
void WCanvas::setHScrollbarLength() {
...
void WCanvas::updateVBarPosition() {
...
void WCanvas::updateHBarPosition() {
