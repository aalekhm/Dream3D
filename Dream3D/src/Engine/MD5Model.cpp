...
Node* MD5Model::loadModel(const char* sFileName) 
{
	...
	if(bCanOpen) 
	{
		...
		while(!pRafIn->isEOF()) 
		{
			...
			if(singleLine.find("numMeshes") == 0)
			{	
				sscanf(singleLine.c_str(), "numMeshes %d", &m_iNumMeshes);
			}
			else
			if(singleLine.find("joints") == 0)
			{	
				Joint* joint = NULL;
			...
			if(singleLine.find("mesh") == 0)
			{
				...
				while(true) 
				{
					...
					if(singleLine.find("shader") == 0) 
					{
						sscanf(singleLine.c_str(), "shader %s", mesh->m_sShader);
					}
					else
					if(singleLine.find("numverts") == 0) 
					{
						Vertex* vert = NULL;
					...
					if(singleLine.find("numweights") == 0) 
					{
						Weight* weight = NULL;
