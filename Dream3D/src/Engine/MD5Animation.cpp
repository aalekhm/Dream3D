...
bool MD5Animation::loadAnimation(const char* sFileName) 
{
	...
	if(bCanOpen) 
	{
		...
		while(!pRafIn->isEOF()) 
		{
			...
			if(singleLine.find("baseframe") == 0) 
			{
			...
			if(singleLine.find("frame") == 0)
			{
				...
				while(true) 
				{
					...
					char* strTok = strtok((char*)singleLine.c_str(), " ");
					while(strTok != NULL) 
					{
