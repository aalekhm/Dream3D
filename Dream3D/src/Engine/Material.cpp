...
void resolveFilenameAndMaterialNamespace(std::string& sUrl, std::string& sFileString, std::string& sNamespace) 
{
	// Find pos of "#".
	int iHashPos = sUrl.rfind("#");
	if (iHashPos >= 0)
	{
...
Material* Material::create(const char* url) {
	...
	std::string sUrlString = url;
	std::string sFilenameString;
