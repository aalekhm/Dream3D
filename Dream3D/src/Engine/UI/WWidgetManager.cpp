...
H_FONT WWidgetManager::loadFont(const char* sFontFile, unsigned int iFontSize, unsigned int iFontDPI) {
	...
	///////////////////////////////////////////////////
	stdReplace(sFontFileName, "tga", "dat");
	...
	while(rafIn->isEOF() == 0) {
		...
		if(singleLine.find("WIDGET") == 0) {
			...
			while(true) 
			{
				...
				if(READ_STATE == STATE_READ_WIDGET && singleLine.find("}") == 0) 
				{
					m_pWidgets.push_back(widget);
					break;
				}
				else
				if(singleLine.find("BaseSkin") == 0) {
					baseSkin = new BASESKIN();
					READ_STATE = STATE_READ_BASESKIN;
				}
				else
				if(singleLine.find("Child") == 0) {
					child = new CHILD();
					READ_STATE = STATE_READ_CHILD;
				}
				else
				if(singleLine.find("ClientArea") == 0) {
					clientArea = new CLIENT_AREA();
					READ_STATE = STATE_READ_CLIENTAREA;
				}
				else
				if(singleLine.find("SimpleText") == 0) {
