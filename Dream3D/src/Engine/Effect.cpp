...
static void replaceDefines(const char* sDefines, std::string& sOut) 
{
	if (sDefines) 
	{
		sOut += "#define ";
...
static void replaceIncludes(const char* pFilePath, const char* pSource, std::string& sOut) {

	// Replace the #include "xxxx.xxx" with the sourced file contents of "pFilepath/xxxx.xxx"
	std::string str = pSource;
	size_t iHeadPos = 0;
	...
	while (iHeadPos < iFileLength) 
	{
		...
		iHeadPos = str.find("#include", (int)iLastHeadPos);
		if (iHeadPos != -1) 
		{
			sOut += str.substr(iLastHeadPos, iHeadPos - iLastHeadPos);
			size_t iStartQuote = str.find("\"", (int)iHeadPos + 1);
			if (iStartQuote != -1) 
			{
				size_t iEndQuote = str.find("\"", (int)iStartQuote + 1);
				if (iEndQuote != -1) 
				{
					iHeadPos = iEndQuote + 1;
					std::string sFilePath = pFilePath;
					std::string sIncludeFile = str.substr(iStartQuote + 1, iEndQuote - (iStartQuote + 1)).c_str();
					std::string sDirectoryPath = sFilePath.substr(0, sFilePath.rfind("/") + 1).c_str();
