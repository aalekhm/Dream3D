...
extern "C" 
{
	...
	public:
		...
		bool openForRead(const char* fileName) 
		{
			...
			if (fileStatus == -1) 
			{
				printf(" %s File/Directory does not exist!!!", fileName);
				return false;
			}
		...
		const char* openForAppend(const char* fileName) 
		{
		...
		bool openForRW(const char* fileName) 
		{
			int fileStatus = _access(fileName, 6);
			if (fileStatus == -1) 
			{
				printf("open --> File/Directory does not exist!!!\n");
				return false;
			}
		...
		void writeByte(const char* byte0) 
		{
			if (m_pFileObj != nullptr) 
			{
		...
		void writeShort(const char* shortValue) 
		{
			if (m_pFileObj != nullptr) 
			{
		...
		void writeBoolean(const char* isTrue) 
		{
			if (m_pFileObj != nullptr) 
			{
				fseek(m_pFileObj, m_lFilePointer, SEEK_SET);
		...
		void writeBoolean(int isTrue) 
		{
			writeByte(isTrue > 0 ? 1 : 0);
		}

		void writeChar(const char* charValue) 
		{
			if (m_pFileObj != nullptr) 
			{
				fseek(m_pFileObj, m_lFilePointer, SEEK_SET);
		...
		void writeChars(const char* buf) 
		{
			if (m_pFileObj != nullptr) 
			{
				fseek(m_pFileObj, m_lFilePointer, SEEK_SET);
		...
		void writeBytes(const char* buf) 
		{
			writeChars(buf);
		}

		void writeIntS(const char* intValue) 
		{
			if (m_pFileObj != nullptr) 
			{
		...
		void writeInt(unsigned long intValue) 
		{
			if (m_pFileObj != nullptr) 
			{
		...
		void writeLong(const char* longValue) 
		{
			if (m_pFileObj != nullptr) 
			{
				fseek(m_pFileObj, m_lFilePointer, SEEK_SET);
		...
		void setLength(const char* newLength) 
		{
			if (m_pFileObj != nullptr) 
			{
		...
		void renameFile(const char* newFileName) 
		{
			if (m_pFileObj != nullptr) 
			{
